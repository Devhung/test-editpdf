<script>
  import { fly } from 'svelte/transition';
  export let message = '';
  export let type = 'error'; // error, warning, success
  export let duration = 3000;
  export let onClose = () => {};

  let visible = true;

  if (duration > 0) {
    setTimeout(() => {
      visible = false;
      onClose();
    }, duration);
  }
</script>

{#if visible}
  <div
    transition:fly={{ y: -50, duration: 300 }}
    class="fixed bottom-0 mb-8 w-full flex justify-center items-center z-50 px-2"
  >
    <div
      class="px-2 py-2 md:px-4 md:py-3 rounded shadow-lg flex items-center max-w-sm md:max-w-md border relative"
      class:bg-red-100={type === 'error'}
      class:border-red-300={type === 'error'}
      class:bg-orange-100={type === 'warning'}
      class:border-orange-300={type === 'warning'}
      class:bg-blue-100={type === 'info'}
      class:border-blue-300={type === 'info'}
      class:bg-green-100={type === 'success'}
      class:border-green-300={type === 'success'}
    >
      <div
        class="mr-3"
        class:text-red-600={type === 'error'}
        class:text-orange-600={type === 'warning'}
        class:text-blue-600={type === 'info'}
        class:text-green-600={type === 'success'}
      >
        {#if type === 'error'}
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
          </svg>
        {:else if type === 'warning'}
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
          </svg>
        {:else if type === 'info'}
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
          </svg>
        {:else}
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
          </svg>
        {/if}
      </div>
      <div
        class="text-sm flex-1"
        class:text-red-800={type === 'error'}
        class:text-orange-800={type === 'warning'}
        class:text-blue-800={type === 'info'}
        class:text-green-800={type === 'success'}
      >
        {message}
      </div>
      <button
        class="ml-4 text-gray-400 hover:text-gray-600 focus:outline-none"
        on:click={() => {
          visible = false;
          onClose();
        }}
      >
        <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
        </svg>
      </button>
    </div>
  </div>
{/if}
